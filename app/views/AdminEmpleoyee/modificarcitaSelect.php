...
<body>
  ...
  ">
    ...
    // Obtener datos enviados por GET (desde la página anterior)
    $idreserva = isset($_GET['id']) ? intval($_GET['id']) : 0;
    $fecha_reserva = isset($_GET['fecha_reserva']) ? $_GET['fecha_reserva'] : '';
    $hora_reserva = isset($_GET['hora_reserva']) ? $_GET['hora_reserva'] : '';
    $estado = isset($_GET['estado']) ? $_GET['estado'] : '';
    $id_categoria = isset($_GET['id_categoria']) ? $_GET['id_categoria'] : '';

    // Consultar la cita actual
    $cita = null;
    if ($idreserva > 0) {
        $stmt = $conexion->prepare("SELECT r.*, e.*, u.* FROM reservas r INNER JOIN empleados e ON r.id_empleado = e.id INNER JOIN usuarios u ON e.id_usuario = u.id WHERE r.id = ?");
    ...
    // Consultar empleados y servicios según la categoría de la cita (si aplica)
    $empleados = [];
    $servicios = [];
    ...
    if ($cita) {
        ...
        // Empleados
        $query = "SELECT s.id, u.nombre, u.apellido FROM empleados s INNER JOIN usuarios u ON u.id = s.id_usuario WHERE s.id_categoria = $id_categoria";
        $resultado = $conexion->query($query);
        while ($row = $resultado->fetch_assoc()) {
            $empleados[] = $row;
        }
        // Servicios
        $query = "SELECT id, nombre FROM servicios WHERE id_categoria = $id_categoria";
        $resultado = $conexion->query($query);
        while ($row = $resultado->fetch_assoc()) {
            $servicios[] = $row;
        }
            ...
            <div class="bg-white rounded-md px-4 py-6 h-max w-auto shadow-sm border border-gray-200">
                <div class="flex items-center justify-center ">
                    ...
                    <div class="mx-auto w-full max-w-[550px] bg-white">
                        <form action="/app/controllers/serviciosAdmin/actualizarCita.php" method="post">
                            <input type="hidden" name="actualizar_reserva_id" value="<?php echo htmlspecialchars($idreserva); ?>">
                            <div class="-mx-3 flex flex-wrap">
                                <div class="w-full px-3 sm:w-1/2">
                                    <div class="mb-5">
                                        ...
                                        <input type="date" name="fecha" id="date" required
                                            ...
                                            value="<?php echo htmlspecialchars(trim($fecha_reserva)); ?>"
                                            />
                                    </div>
                                </div>
                                <div class="w-full px-3 sm:w-1/2">
                                    <div class="mb-5">
                                        ...
                                        <input type="time" name="hora" id="time" required
                                            class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md" 
                                            value="<?php echo htmlspecialchars(trim($hora_reserva)); ?>"
                                            />
                                ...
                                <div class="w-full px-3 sm:w-2/2">
                                    <div class="mb-5">
                                        <label for="estilistas" class="mb-3 block text-base font-medium text-[#07074D]">
                                            Estilistas
                                        </label>
                                        <div class="relative w-max mx-auto">
                                            <select name="estilistas" id="estilistas" required class="w-full rounded-md border py-3 px-6 text-base">
                                                <option value="">Seleccione un estilista</option>
                                                <?php
                                                $query = "SELECT u.nombre AS nombreEmpleado, u.apellido AS apellidoEmpleado, e.id AS idEmpleado FROM empleados e INNER JOIN usuarios u ON u.id = e.id_usuario WHERE e.id_categoria = $id_categoria";
                                                $resultado = $conexion->query($query);

                                                // Obtener el id_empleado de la reserva actual (si existe)
                                                $selectedEmpleado = isset($cita['id_empleado']) ? $cita['id_empleado'] : '';

                                                if ($resultado && $resultado->num_rows > 0) {
                                                    while ($row = $resultado->fetch_assoc()) {
                                                        $selected = ($row['idEmpleado'] == $selectedEmpleado) ? "selected" : "";
                                                        echo "<option value='" . $row['idEmpleado'] . "' $selected>" . htmlspecialchars($row['nombreEmpleado']) . " " . htmlspecialchars($row['apellidoEmpleado']) . "</option>";
                                                    }
                                                } else {
                                                    echo "<option value=''>No hay estilistas disponibles</option>";
                                                }
                                                ?>
                                            </select>
                                        </div>
                                    ...
                                    <div class="mb-5">
                                        ...
                                        <div class="relative w-max mx-auto">
                                            ...
                                            <select name="id_servicio" id="id_servicio" required class="w-full rounded-md border py-3 px-6 text-base">
                                                ...
                                                <?php
                                                // Obtener el id_servicio de la reserva actual (si existe)
                                                $selectedServicio = isset($_GET['id_servicio']) ? intval($_GET['id_servicio']) : (isset($cita['id_servicio']) ? $cita['id_servicio'] : '');

                                                $query = "SELECT id, nombre FROM servicios WHERE id_categoria = $id_categoria";
                                                $resultado = $conexion->query($query);
                                                if ($resultado && $resultado->num_rows > 0) {
                                                    while ($row = $resultado->fetch_assoc()) {
                                                        $selected = ($row['id'] == $selectedServicio) ? "selected" : "";
                                ...
                                <div class="w-full px-3 sm:w-2/2">
                                    <div class="mb-5">
                                        ...
                                        <div class="relative w-max mx-auto">
                                            <select name="estado" id="estado" required class="w-full rounded-md border py-3 px-6 text-base">
                                                <?php
                                                // Estados posibles
                                                $estados = [
                                                    'pendiente' => 'pendiente',
                                                ...
                                                $selectedEstado = isset($_GET['estado']) ? $_GET['estado'] : (isset($cita['estado']) ? $cita['estado'] : '');
                                                foreach ($estados as $valor => $texto) {
                                                    $selected = ($valor == $selectedEstado) ? "selected" : "";
